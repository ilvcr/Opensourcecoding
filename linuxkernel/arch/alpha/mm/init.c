...
> Created Time: 2018年08月07日 星期二 23时41分58秒
> Description: 
 ************************************************************************/
...
static inline unsigned long load_PCB(struct thread_struct* pcb){
...
static inline void switch_to_system_map(void){
    ...
    pfd_val(swapper_pg_dir[1023]) = (newptbr << 32) | pgprot_val(PAGE_KERNEL);
    ...
    if(original_pcb_ptr < PAGE_OFFSET){
        ...
        original_pcb_ptr = (unsigned long) phys_to_virt(original_pcb_ptr);
    }
...
void* __init callback_init(void* kernel_end){
    ...
    if(alpha_using_srm){
        ...
        crb->dispatch_va = (struct procdesc_struct*)
            (VMALLOC_START + (unsigned long)crb->dispatch_va - crb->map[0].va);
        
        crb->fixup_va = (struct procdesc_struct *)
            (VMALLOC_START + (unsigned long)crb->fixup_va - crb->map[0].va);
    }
    ...
    two_pages = (void*) (((unsigned long) kernel_end + ~PAGE_MASK) & PAGE_MASK);
    ...
    if(alpha_using_srm){
        ...
        /* Set up the third level PTEs and update the virtual addresses of the CRB entries.  */
        for(i=0; i < crb->map_entries; ++i){
            ...
            for(j=0; j < crb->map[i].count; ++j){
                ...
                set_pte(pte_offset(pmd, vaddr), mk_pte_phys(paddr, PAGE_KERNEL));
                paddr += PAGE_SIZE;
                vaddr += PAGE_SIZE;
            }
        }
...
void si_meminfo(struct ayainfo* val){
