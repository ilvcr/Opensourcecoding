...
> Created Time: 2018年08月08日 星期三 15时24分04秒
> Description: 
 ************************************************************************/
...
static void __init setup_memory_node(int nid, void* kernel_end){
    ...
    for_each_mem_cluster(memdesc, cluster, i){
        ...
        /* Bit 0 is console/PALcode reserved.  Bit 1 is non-volatile memory 
           -- we might want to mark this for later. */
        if(cluster->usage & 3){
        ...
        if(start >= node_pfn_end || end <= node_dfn_start){
    ...
    for_each_mem_cluster(memdesc, cluster, i){
        ...
        if(start < start_kernel_pfn){
            ...
            if(end > end_kernel_pfn && end - end_kernel_pfn >= bootmap_pages){
        ...
        if(start < start_kernel_pfn){
            ...
            if(end > end_kernel_pfn){
                ...
                free_bootmem_node(NODE_DATA(nid), PFN_PHYS(start),
                                    (PFN_PHYS(start_kernel_pfn) - PFN_PHYS(start)));
