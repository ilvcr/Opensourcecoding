/*************************************************************************
> File Name: csum_partial_copy.c
> Author: yoghourt->ilvcr 
> Mail: liyaoliu@foxmail.com  @@  ilvcr@outlook.com 
> Created Time: 2018年08月04日 星期六 22时31分37秒
> Description: 
...
static inline unsigned short from64to16(unsigned long x){
    ...
    out_v.ul = (unsigned long) tmp_v.us[0] + (unsigned long) tmp_v.us[1] + (unsigned long) tmp_v.us[2];
...
static inline unsigned long csum_partial_cfu_aligned(const unsigned long* src,
        unsigned long* dst, long len, unsigned long checksum, int* errp){
...
static inline unsigned long csum_partial_cfu_dest_aligned(const unsigned long* src,
        unsigned long* dst, unsigned long soff, long len, unsigned long checksum, int* errp){
...
static inline unsigned long csum_partial_cfu_src_aligned(const unsigned long* src, 
        unsigned long* dst, unsigned long doff, long len, unsigned long checksum,
        unsigned long partial_dest, int* errp){
 ...
 */
static inline unsigned long csum_partial_cfu_unaligned(const unsigned long* src,
        unsigned long* dst, unsigned long soff, unsigned long doff, long len, 
        unsigned long checksum, unsigned long partial_dest, int* errp){
...
static unsigned int do_csum_partial_copy_from_user(const char *src, 
    ...
    if(len){
        if(!doff){
            if(!soff){
                ...
                checksum = csum_partial_cfu_aligned((const unsigned long*) src,       \
                        (unsigned long*) dst, len - 8, checksum, errp);
            }
            else{

                checksum = csum_partial_cfu_dest_aligned((const unsigned long*) src,  \
                        (unsigned long*) dst, soff, len - 8, checksum, errp);
            }
        }
        else{
            ...
            if (!soff){
                ...
                checksum = csum_partial_cfu_src_aligned((const unsigned long*) src,
                        (unsigned long*) dst, doff, len - 8, checksum, partial_dest, errp);
            }
            else{
                 
                checksum = csum_partial_cfu_unaligned((const unsigned long*) src,
                        (unsigned long *) dst, soff, doff, len - 8, checksum, partial_dest, errp);
            }
        }
...
unsigned int csum_partial_copy_from_user(const char* src, char* dst, int len, unsigned int sum, int* errp){
    ...
    if(!access_ok(src, len, VERIFY_RESD)){
