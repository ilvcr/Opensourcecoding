...
> Created Time: 2018年08月05日 星期日 13时03分49秒
> Description: Alpha IO and memory functions.. Just expand the inlines in the header files..
 ************************************************************************/
...
void _memcpy_fromio(void* to, unsigned long from, long count){
    ...
    /* Optimize co-aligned transfers.  Everything else gets handled a byte at a time. */
...
void scr_memcpyw(u16* d, const u16* s, unsigned int count){
    ...
    if(! __is_ioaddr((unsigned long) s)){
        ...
        if(! __is_ioaddr((unsigned long) d)){
            ...
            /* FIXME: Should handle unaligned ops and operation widening.  */
            count /= 2;
            while (count--) {
