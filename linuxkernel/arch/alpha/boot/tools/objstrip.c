...
> Created Time: 2018年07月29日 星期日 22时38分10秒
> Description: Strip the object file headers/trailers from an executable (ELF or ECOFF).
...
int main(int argc, char* argv[]){
    ...
    size_t nwritten, tocopy, n, mem_size, filesize, pad = 0;
    ...
    if(elf->e_ident[0] == 0x7f && strncmp(elf->e_ident+1, "ELF", 3) == 0){
        ...
        if(verbose){
            ...
            fprintf(stderr, "%s: extracting %#016lx-%#016lx (at %lx)\n", prog_name, (long) elf_phdr->p_vaddr, elf_phdr->p_vaddr + fil_size, offset);
        }
    }
    else{
#endif
        ...
        if(aout->fh.fh.f_opthdr != sizeof(aout->ah)){
        ...
        if(verbose){
            ...
            fprintf(stderr, "%s: extracting %#016lx-%#016lx (at %lx)\n",  rog_name, aout->ah.text_start, aout->ah.text_start + fil_size, offset);
        }
    }
