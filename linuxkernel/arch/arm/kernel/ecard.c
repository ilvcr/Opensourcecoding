...
> Created Time: 2018年08月16日 星期四 14时56分43秒
> Description: 
...
#define POD_INT_ADDR(x) ((volatile unsigned char *) ((BUS_ADDR((x)) - IO_BASE) + IO_START))
            ...
            while(len--){
                ...
                *buf++ = base_addr[off];
                ...
                while(len--){
                    ...
                    /* The following is required by some expansion card loader programs. */
                    *(unsigned long*)0x108 = 0;
                    *buf++ = ecard_loader_read(off++, base_addr, req->ec->loader);
                }
            }
        }
    }
...
static int ecard_taask(void* unused){
...
static void ecard_call(struct ecard_request* req){
    ...
    if(ecard_pid <= 0){
        ...
        ecard_pid = kernel_thread(ecard_task, NULL, CLONE_FS | CLONE_FILES | CLONE_SIGHAND);
    }
...
static void ecard_def_irq_disable(ecard_t* ec, int irqne){

#ifdef HAS_EXPMASK

    if(irq < 4 && have_expmask){
...
static void ecard_dump_irq_state(ecard_t* ec){

    printk("  %d: %sclaimed, ", ec->slot_no,
                                ec->claimed ? "" : "not ");

    if(ec->ops && ec->ops->irqpending && ec->ops != &ecard_default_ops){
...
static void ecard_check_lockup(void){
     ...
     *                  
     *  Maybe we ought to start a timer to re-enable them some time later?
     */
    if(last == jiffies){
...
static int ecard_prints(char* buffer, ecard_t* ec){
    ...
    buffer += sprintf(buffer, "  %d: %s ", ec->slot_no, ec->type == ECARD_EASI ? "EASI" : "    ");

    if(ec->cid.id == 0){

        struct in_chunk_dir incd;

        buffer += sprintf(buffer, "[%04X:%04X] ", ec->cid.manufacturer, ec->cid.product);

        if(!ec->card_desc && ec->cid.cd && ecard_readchunk(&incd, ec, 0xf5, 0)){
 ...
 *  card does not exist.
 */
static int __init ecard_probe(int slot, card_type_t type){
