...
> Created Time: 2018年08月20日 星期一 19时57分30秒
> Description: 
...
static void check_irq_lock(struct irqdesc* desc, int irq, struct pt_regs* regs){
    ...
    if(desc->lck_jif == jiffie && desc->lck_pc >= instr_ptr && desc->lck_pc < instr_ptr + 8){
...
int request_irq(unsigned int irq, void (*handler)(int, void*, struct pt_regs*),
                            unsigned long irq_flags, const char* devname, void*mdev_id){
...
unsigned long probe_irq_on(void){
    ...
    for(i=0; i < NR_IRQS; i++){
        ...
        if(!irq_desc[i].valid || !irq_desc[i].probe_ok || irq_desc[i].action){
