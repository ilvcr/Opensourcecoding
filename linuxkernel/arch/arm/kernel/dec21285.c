/*************************************************************************
> File Name: dec21285.c
> Author: yoghourt->ilvcr 
> Mail: liyaoliu@foxmail.com  @@  ilvcr@outlook.com 
> Created Time: 2018年08月14日 星期二 22时48分56秒
> Description: 
...
static unsigned long dc21285_base_address(struct pci_dev* dev){
    ...
    if(dev->bus->number == 0){
        ...
        if(PCI_SLOT(devfn) == 0){
            ...
            if(devfn < PCI_DEVFN(MAX_SLOTS, 0)){
                ...
                addr = PCICMD_BASE | 0xc00000 | (devfn << 8);
            }
        }
    }
    else{
...
static int dc21285_read_config_byte(struct pci_dev* dev, int where, u8* value){
    ...
    if(addr){
        ...
        asm("ldr%?b %0, [%1, %2]" : "=r" (v) : "r" (addr), "r" (where));
    }
    else{
...
static int dc21285_read_config_word(struct pci_dev* dev, int where, u16* value){
    ...
    if(addr){
        ...
        asm("ldr%?h %0, [%1, %2]" : "=r" (v) : "r" (addr), "r" (where));
    }
    else{
...
static int dc21285_read_config_dword(struct pci_dev* dev, int where, u32* value){
    ...
    if(addr){
        ...
        asm("ldr%?  %0, [%1, %2]" : "=r" (v) : "r" (addr), "r" (where));
    }
    else{
...
static int dc21285_read_config_byte(struct pci_dev* dev, int where, u8 value){
    ...
    if(addr){
        ...
        asm("str%?b %0, [%1, %2]" : : "r" (value), "r" (addr), "r" (where));
    }
...
static int dc21285_write_config_word(struct pci_dev* dev, int where, u16 value){

    unsigned long addr = dc21285_base_address(dev);

    if(addr){

        asm("str%?h %0, [%1, %2]" : : "r" (value), "r" (addr), "r" (where));
    }
...
static int dc21285_write_config_dword(struct pci_dev* dev, int where, u32 value){

    unsigned long addr = dc21285_base_address(dev);

    if(addr){

        asm("str%?  %0, [%1, %2]" : : "r" (value), "r" (addr), "r" (where));
    }
...
void __init dc21285_setup_resources(struct resource** resource){
    ...
    allocate_resource(&iomem_resource, busmempf, 0x20000000, 0x80000000, 0xffffffff, 0x20000000, NULL, NULL);
    allocate_resource(&iomem_resource, busmem, 0x40000000, 0x80000000, 0xffffffff, 0x40000000, NULL, NULL);
...
void __init dc21285_init(void* sysdata){
    ...
    /*These registers need to be set up whether we're the central function or not. */
    ...
    *CSR_PCIADDR_EXTN     = 0;
    ...
    if(cfn_mode){
        ...
        static struct resource csrmem, csio;
        ...
        allocate_resource(&ioport_resource, &csrio, 128, 0xff00, 0xffff, 128, NULL, NULL);
        allocate_resource(&iomem_resource, &csrmem, 128, 0xf4000000, 0xf8000000, 128, NULL, NULL);
        ...
        /* Clear any existing errors - we aren't interested in historical data... */
        ...
        *CSR_PCICMD = (*CSR_PCICMD & 0xffff) | PCICMD_ERROR_BITS;
         ...
         * set up the machine.
         */
        panic("PCI: this kernel is compiled for central ""function mode only");
    }
    ...
    request_irq(IRQ_PCI_SERR, dc21285_serr_irq, SA_INTERRUPT, "PCI system error", &serr_timer);
    request_irq(IRQ_PCI_PERR, dc21285_parity_irq, SA_INTERRUPT, "PCI parity error", &perr_timer);
