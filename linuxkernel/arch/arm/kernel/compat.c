...
> Created Time: 2018年08月13日 星期一 14时45分53秒
> Description: 
...
static void __init build_tag_list(struct param_struct* params, void* taglist, int mem_init){
    ...
    tag->hdr.size = (strlen(params->commandline) + 3 + sizeof(struct tag_header)) >> 2;
    ...
    memmove(params, taglist, ((int)tag) - ((int)taglist) + sizeof(struct tag_header));
}
