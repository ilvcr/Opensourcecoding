...
> Created Time: 2018年08月16日 星期四 14时17分28秒
> Description: 
...
static void iomd_dma_handle(int irq, void* dev_id, struct pt_regs* regs){
    ...
    unsigned itn status = 0, no_buffer = dma->sg == NULL;
...
static int iomd_request_dma(dmach_t channel, dma_t* dma){
    ...
    return request_irq(dma->dma_irq, iomd_dma_handle, SA_INTERRUPT, dma->device_id, dma);
}
...
static void iomd_enable_dma(dmach_t channel, dma_t* dma){
    ...
    if(dma->invalid){
        ...
        /* Cope with ISA-style drivers which expect cache coherence. */
        if(!dma->using_sg){

            dma->buf.dma_address = pci_map_single(NULL, dma->buf.address, dma->buf.length,
                    dma->dma_mode == DMA_MODE_READ ? PCI_DMA_FROMDEVICE : PCI_DMA_TODEVICE);
        }
...
static int iomd_set_speed(dmach_t channel, dma_t* dma, int cycle){
