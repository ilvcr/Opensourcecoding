...
> Created Time: 2018年08月13日 星期一 13时40分48秒
> Description: 
 ************************************************************************/
...
struct pci_sys_data{
    ...
    /* The hardware we are attched to */
    struct hw_pci* hw;

    unsigned long mem_offset;

    /* These are the resources for the root bus */
    struct resources* resource[3];
};
...
void pcibios_report_status(u_int status_mask, int warn){
    ...
    pci_for_each_dev(dev){
        ...
        u16 status;
        /* ignore host bridge - we handle that separately */
...
static inline int pdev_bad_for_parity(struct pci_dev* dev){
    ...
    return (dev_>vendor == PCI_VENDOR_ID_INTERG &&
...
static void __init pdev_fixup_device_resources(struct pci_sys_data* root, 
                                               struct pci_dev* dev){
...
static void __init pbus_assign_bus_resources(struct pci_bus* bus, 
...
void __init pcibios_fixup_bus(struct pci_bus* bus){
    ...
    for(walk=bus->devices.next; walk != &bus->devices; walk=walk->next){
         ...
         * flag.  We will then go looking for things like keyboard, etc
         */
     ...
     * Having fast back to back transfers on saves us one PCI cycle per transaction.
     */
...
char* __init pcibios_setup(cahr* str){
