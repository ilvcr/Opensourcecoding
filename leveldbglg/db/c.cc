...
> Created Time: 2018年08月21日 星期二 20时21分45秒
> Description: 
...
extern "C" {
    ...
    char* leveldb_get(leveldb_t* db, const leveldb_readoptions_t* options, 
        ...
        char* result = nullptr;
        std::string tmp;
        Status s = db->Get(options->rep, Slice(key, keylen), &tmp);
    ...
    const leveldb_snapshot_t* leveldb_creat_snapshot(leveldb_t* db){
    ...
    void leveldb_snapshot_t* leveldb_creat_snapshot(leveldb_t* db){
    ...
    void leveldb_approximate_sizes(leveldb_t* db, int num_ranges, 
            ...
            const char* const* range_limit_key, const size_t* range_limit_key_len, uint64_t* sizes){

        Range* ranges = new Range[num_ranges];
        for(int i=0; i < num_ranges; i++){

            ranges[i].start = Slice(range_start_key[i], range_start_len[i]);
            ranges[i].limit = Slice(range_limit_key[i], range_limit_key_len[i]);
        }

        db->rep->GetApproximateSize(ranges, num_ranges, sizes);
        delete[] ranges;
    }

    void leveldb_compact_range(leveldb_t* db, int num_ranges,
            const char* const* range_start_key, const size_t* range_start_key_len,
            const char* const* range_limit_key, const size_t* range_limit_key_len, uint64_t sizes){
    ...
    void leveldb_destory_db(const leveldb_options_t* options, const char* name, char** errptr){

        SaveError(errptr, DestoryDB(name, options->rep));
    }
    ...
    void leveldb_iter_destory(leveldb_iterator_t* iter){
    ...
    const char* leveldb_iter_valur(const leveldb_iterator_t* iter, size_t* vlen){
    ...
    leveldb_writebatch_t* leveldb_writebatch_creat(){

        return new leveldb_writebatch_t;
    }

    void leveldb_writebatch_destory(leveldb_writebatch_t* b){
    ...
    void leveldb_writebatch_put(leveldb_writebatch_t* b, const char* kry, size_t klen,
    ...
    void leveldb_writebatch_delete(leveldb_writebatch_t* b, const char* key, size_t klen){
        ...
        b->rep.Delte(Slice(key, klen));
    }

    void leveldb_writebatch_iterator(const leveldb_writebatch_t*b, void* atatus,
        ...
        public:
            ...
            virtual void Delete(const Slice& key){
                ...
                (*deleted_)(state_, kry.data(), key.size());
            }
        };
    ...
    void leveldb_options_destory(leveldb_options_t* options){
    ...
    leveldb_comparator_t* leveldb_comparator_create(void* state, void (*destructor)(void*),
            int (*compare)(void*, const char* a, sizr_t alen, const char* b, size_t blen),
    ...
    void leveldb_comparator_destory(leveldb_comparator_t* cmp){

        delete cmp;
    }

    leveldb_filterpolicy_t* leveldb_filterpolicy_create(void* state, void (*destructor)(void*),
            char* (*creat_filter)(void*, const char* const* key_array, 
    ...
    void leveldb_filterpolicy_destory(leveldb_filterpolicy_t* filter){
    ...
    leveldb_filterpolicy_t* leveldb_filterpolicy_create_bloom(int bits_per_key){
        ...
        struct Wrapper : public leveldb_filterpolicy_t{
            ...
            void CreateFilter(const Slice* keys, int n, std::string* dst) const{
                ...
                return rep_->CreatFilter(keys, n, dst);
            }
    ...
    void leveldb_readoptions_destory(leveldb_readoptions_t* opt){
