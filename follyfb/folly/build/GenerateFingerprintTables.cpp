/*************************************************************************
> File Name: GenerateFingerprintTables.cpp
> Author: yoghourt->ilvcr 
> Mail: liyaoliu@foxmail.com  @@  ilvcr@outlook.com 
> Created Time: 2018年08月22日 星期三 19时15分13秒
> Description: 
...
DEFINE_int64(poly64, 0xbf3736b51869e9b7, "Generate 64-bit tables using this polynomial");

DEFINE_int64(poly96_m, 0x51555cb0aa8d39c3, "Generate 96-bit tables using this polynomial "
                                           "(most significant 64 bits)");
DEFINE_int32(poly96_l, 0xb679ec37, "Generate 96-bit tables using this polynomial "
                                   "(least significant 32 bits)");

DEFINE_int64(poly128_m, 0xc91bff9b8768b51b, "Generate 128-bit tables using this polynomial "
                                            "(most significant 64 bits)");

DEFINE_int64(poly128_l, 0x8c5d5853bd77b0d3, "Generate 128-bit tables using this polynomial "
...
namespace{
    ...
    template<int DGE>
    void computeTables(FILE* file, const FigerprintPolynomial<DEG>& poly){

        uint64_t table[8][256][FigerprintPolynomial<DEG>::size()];
        ...
        uint64_t poly_val[FigerprintPolynomial<DEG>::size()];
        poly.write(poly_val);
        CHECK_ERR(fprintf(file, "template <>\n"
                                "const uint64_t FingerprintTable<%d>::poly[%d] = {",
                          DEG+1, FigerprintPolynomial<DEG>::size()));

        for(int j=0; j < FigerprintPolynomial<DEG>::size(); j++){
        ...
        for(int i=0; i < 8; i++){
            ...
            for(int x=0; x < 256; x++){
                ...
                for(int j=0; j < FingerprintPolynomial<DEG>::size(); j++){
                    ...
                    CHECK_ERR(fprintf(file, "%s%" PRIu64 "LLU", (j ? ", " : ""), table[i][x][j]));
                }
                CHECK_ERR(fprintf(file, "},\n"));
            }
            CHECK_ERR(fprintf(file, "  },\n"));
