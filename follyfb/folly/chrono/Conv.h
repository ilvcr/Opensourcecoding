...
> Created Time: 2018年08月22日 星期三 23时09分24秒
> Description: 
...
namespace folly{
    namespace detail{
        ...
        struct is_chrono_conversion{
            ...
            static constexpr bool value = ((is_std_chrono_type<Tgt>::value && 
        ...
        Expected<time_t, ConversionCode> 
        chronoRangeCheck(Src value){
        ...
        static Expected<std::pair<time_t, long>, ConversionCode> 
        durationToPosixTime(const std::chrono::duration<Rep, std::ratio<1, 1>>& duration){
          
            static_assert(SubsecondRatio::num == 1, "subsecond numerator should always be 1");
        ...
        static Expected<std::pair<time_t, long>, ConversionCode> 
        durationToPosixTime(const std::chrono::duration<Rep, std::ratio<1, Denominator>>& duration){
        ...
        static Expected<std::pair<time_t, long>, ConversionCode> 
        durationToPosixTime(const std::chrono::duration<Rep, std::ratio<Numerator, 1>>& duration){
        ...
        template<typename Rep, typename Period>
        using IntermediateDuration = std::chrono::duration<typename IntermediateTimeRep<Rep,
        ...
        Expected<std::pair<time_t, long>, ConversionCode> 
        durationToPosixTime(const std::chrono::duration<Rep, Period>& duration){
            ...
            // type.
            constexpr auto maxInput = std::numeric_limits<IntermediateRep>::max() / Period::num;
            ...
            constexpr auto minInput = std::numeric_limits<IntermediateRep>::min() / Period::num;
          
            if(duration.count() < minInput){
            
                return makeUnexpected(ConversionCode::NEGATIVE_OVERFLOW);
            }
            auto intermediate = IntermediateType{static_cast<IntermediateRep>(duration.count()) *
        ...
        struct CheckOverflowToDurationx{
            ...
            static ConversionCode check(Seconds seconds, Subseconds subseconds){
                ...
                static_assert(Tgt::period::num == 1, 
                              "this implementation should only be used for subsecond granularity "
                              "duration types");
                static_assert(!std::is_floating_point<typename Tgt::rep>::value, "incorrect usage");
                static_assert(SubsecondRatio::num == 1, "subsecond numerator should always be 1");
                ...
                if(LIKELY(seconds >= 0)){
                    ...
                    if(UNLIKELY(unsignedSeconds == maxSeconds)){
                        ...
                        constexpr auto maxSubseconds = (maxRemainder * SubsecondRatio::den) / Tgt::period::den;
        ...
        struct CheckOverflowToDuration<true>{
            ...
            static ConversionCode check(
                ...
                // integer value here.
                static_assert(std::numeric_limits<typename Tgt::rep>::max() >=
                              std::numeric_limits<Seconds>::max(),
                              "unusually limited floating point type");

                static_assert(std::numeric_limits<typename Tgt::rep>::lowest() <=
        ...
        auto posixTimeToDuration(Seconds seconds, Subseconds subseconds,
            ...
            if(std::is_floating_point<typename Tgt::rep>::value){
                ...
                return Tgt{typename Tgt::rep(seconds) + (typename Tgt::rep(subseconds) / SubsecondRatio::den)};
            }

            // If the value is negative, we have to round up a non-zero subseconds value
            if(UNLIKELY(outputSeconds.value() < 0) && subseconds > 0){
            
                if(UNLIKELY(outputSeconds.value() == std::numeric_limits
                                                                <typename Tgt::rep>::lowest())){
        ...
        template<typename SubsecondRatio, typename Seconds, typename Subseconds,
                 typename Rep, std::intmax_t Denominator>
        auto posixTimeToDuration(Seconds seconds, Subseconds subseconds,
                                 std::chrono::duration<Rep, 
                                 std::ratio<1, Denominator>> dummy) -> Expected<decltype(dummy), 
                                 ConversionCode>{
            ...
            auto errorCode = detail::CheckOverflowToDuration<std::is_floating_point
                <typename Tgt::rep>::value>::template check<Tgt, SubsecondRatio>(seconds, subseconds);
            ...
            if(LIKELY(seconds >= 0)){
                ...
                return std::chrono::duration_cast<Tgt>(
                       ...
                       std::chrono::duration<typename Tgt::rep, SubsecondRatio>{subseconds});
            } 
            else{
                ...
                return std::chrono::duration_cast<Tgt>(
                       ...
                       std::chrono::duration_cast<Tgt>(
                       std::chrono::duration<typename Tgt::rep, 
                       SubsecondRatio>{SubsecondRatio::den - subseconds});
            }
        }
         ...
         */
        template<typename SubsecondRatio, typename Seconds, typename Subseconds,
                 typename Rep, std::intmax_t Numerator>
        auto posixTimeToDuration(Seconds seconds, Subseconds subseconds,
                                 std::chrono::duration<Rep, 
                                 std::ratio<Numerator, 1>> dummy) -> Expected<decltype(dummy), 
                                 ConversionCode>{
         ...
         */
        template<typename SubsecondRatio, typename Seconds, typename Subseconds, typename Rep,
                 std::intmax_t Denominator, std::intmax_t Numerator>
        auto posixTimeToDuration(Seconds seconds, Subseconds subseconds,
                                 std::chrono::duration<Rep, 
                                 std::ratio<Numerator, 
                                 Denominator>> dummy) -> Expected<decltype(dummy), 
                                 ConversionCode>{
          
            using Tgt = decltype(dummy);
            static_assert(Tgt::period::num != 1, "should use special-case code when num==1");
            static_assert(Tgt::period::den != 1, "should use special-case code when den==1");
            static_assert(SubsecondRatio::num == 1, "subsecond numerator should always be 1");
            ...
            using IntermediateType = IntermediateDuration<typename Tgt::rep, typename Tgt::period>;
            auto intermediate = posixTimeToDuration<SubsecondRatio>(seconds, subseconds, IntermediateType{});
            ...
            return tryTo<typename Tgt::rep>(intermediate.value().count() / 
                   Tgt::period::num).then([](typename Tgt::rep tgt) { return Tgt{tgt}; });
        }


        template <typename Tgt, typename SubsecondRatio, typename Seconds, typename Subseconds>
        Expected<Tgt, ConversionCode> 
        tryPosixTimeToDuration(Seconds seconds, Subseconds subseconds){
            ...
            if(UNLIKELY(subseconds < 0)){
                ...
                if(std::numeric_limits<Seconds>::lowest() + 1 - overflowSeconds >seconds){
    ...
    typename std::enable_if<detail::is_duration<Tgt>::value, Expected<Tgt, ConversionCode>>::type
     ...
     */
    template<typename Tgt, typename Src>
    typename std::enable_if<detail::is_time_point<Tgt>::value && detail::is_posix_time_type<Src>::value,
                            Expected<Tgt, ConversionCode>>::type tryTo(const Src& value){
      
        return tryTo<typename Tgt::duration>(value).then([](typename Tgt::duration result){ 
                return Tgt(result); 
                });
    }
    ...
    typename std::enable_if<std::is_same<Tgt, struct timespec>::value, 
     ...
     */
    template<typename Tgt, typename Rep, typename Period>
    typename std::enable_if<std::is_same<Tgt, struct timeval>::value, 
                            Expected<Tgt, ConversionCode>>::type
                            tryTo(const std::chrono::duration<Rep, Period>& duration){
    ...
    typename std::enable_if<detail::is_posix_time_type<Tgt>::value,
                            Expected<Tgt, ConversionCode>>::type
     ...
     */
    template<typename Tgt, typename Src>
    typename std::enable_if<detail::is_chrono_conversion<Tgt, Src>::value, Tgt>::type 
                                                                        to(const Src& value){
      
        return tryTo<Tgt>(value).thenOrThrow([](Tgt res) { return res; },
