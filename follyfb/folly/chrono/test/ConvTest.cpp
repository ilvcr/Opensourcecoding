...
> Created Time: 2018年08月22日 星期三 19时38分07秒
> Description: 
...
namespace{
    ...
    typename Clock::time_point createTimePoint(const Duration& d){
        ...
        return typename Clock::time_point(std::chrono::duration_cast<typename Clock::duration>(d));
    }
}      // namespace

TEST(Conv, timespaecToStdChrono){
    ...
    ts.tv_nsec = 123456;
    EXPECT_EQ(-1h, to<hours>(ts));
    EXPECT_EQ(-1, duration_cast<hours>(seconds{ts.tv_sec} + nanoseconds{ts.tv_nsec}).count());
    ts.tv_sec = -7201;
    ts.tv_nsec = 123456;
    EXPECT_EQ(-2h, to<hours>(ts));
    EXPECT_EQ(-2, duration_cast<hours>(seconds{ts.tv_sec} + nanoseconds{ts.tv_nsec}).count());
    ...
    // Test with unusual durations where neither the numerator nor denominator are 1
...
TEST(Conv, timespecToStdChronoOverflow){
    ...
    ts.tv_nsec = 999999999;
    EXPECT_EQ(std::numeric_limits<decltype(ts.tv_sec)>::max() / 3600, to<hours_u64>(ts).count());
...
TEST(Conv, timevalToStdChrono){
    ...
    EXPECT_EQ(createTimePoint<system_clock>(1000010000ns),
              to<system_clock::time_point>(tv));
    
    EXPECT_EQ(createTimePoint<steady_clock>(1000010000ns),
    ...
    EXPECT_EQ(createTimePoint<system_clock>(nanoseconds(8219876000LL)),
              to<system_clock::time_point>(tv));
    
    EXPECT_EQ(createTimePoint<steady_clock>(nanoseconds(8219876000LL)),
              to<steady_clock::time_point>(tv));

    // Test for overflow.
    if(std::numeric_limits<decltype(tv.tv_sec)>::max() >= std::numeric_limits<int64_t>::max()){
