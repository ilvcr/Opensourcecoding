/*************************************************************************
> File Name: accumulator_set.hpp
> Author: yoghourt->ilvcr 
> Mail: liyaoliu@foxmail.com  @@  ilvcr@outlook.com 
> Created Time: 2018年07月29日 星期日 11时14分53秒
> Description: 
    ...
    namespace accumulators{
        namespace detail{
            ...
            struct accumulator_visitor{
                ...
                explicit accumulator_visitor(Args const& a) : args(a){
                }
            ...
            template<typename Args>
            inline accumulator_visitor<Args> const make_accunulator_visitor(Args const& Args){

                return accumulator_visitor<Args>(args);
            }

            typedef parameter::parameters<parameter::required<tag::accumulator>, parameter::optional<tag::sample> >
            ...
            // is_accumulator_set
            template<typename T>
            struct is_accumulator_set : is_base_and_derived<accumulator_set_base, T>{
            };
        } // namespace detail
        ...
        struct accumulator_set : detail::accumulator_set_base{
            ...
            //INTERNAL ONLY
            typedef typename detail::make_accunulator_tuple<Features, Sample, Weight>::type accumulators_mpl_vector;
            ...
            accumulator_set() : accumulators(detail::make_acc_list(                                        \
                        accumulators_mpl_vector(), detail::accumulator_params()(*this))){
            ...
            explicit accumulator_set(A1 const& a1) : accumulators(detail::make_acc_list(                   \
                        accumulators_mpl_vector(), detail::accumulator_params()(*this, a1))){
            ...
            #define BOOST_ACCUMULATORS_ACCUMULATOR_SET_CTOR(z, n, _)
                ...
                accumulator_set(BOOST_PP_ENUM_PARAMS_Z(z, n, A, const& a)) : accumulators(                 \
                        detail::make_acc_list(accumulators_mpl_vector(), detail::accumulator_params()(     \
                                *this BOOST_PP_ENUM_PARAMS_Z(z, n, a)))){
                ...
                // INTERNAL ONLY
                BOOST_PP_REPEAT_FROM_TO(2, BOOST_PP_INC(BOOST_ACCUMULATORS_MAX_ARGS),                     \
                    BOOST_ACCUMULATORS_ACCUMULATOR_SET_CTOR, _)
                ...
                void operator ()(){
                    ...
                    this->visit(detail::make_accunulator_visitor(detail::accumulator_params()(*this)));
                }

                template<typename A1>
                void operator ()(A1 const& a1){

                    this->visit(detail::make_accunulator_visitor(detail::accumulator_params()(*this, a1)));
                }
            ...
            #define BOOST_ACCUMULATORS_ACCUMULATOR_SET_FUN_OP(z, n, _)
                ...
                void operator ()(BOOST_PP_ENUM_BINARY_PARAMS_Z(z, n, A, const& a)){
                    ...
                    this->visit(detail::make_accunulator_visitor(detail::accumulator_params()(            \
                        *this BOOST_PP_ENUM_TRALING_PARAMS_Z(z, n, a))));
                }

                // INTERNAL ONLY
                BOOST_PP_REPEAT_FROM_TO(2, BOOST_PP_INC(BPPST_ACCUMULATORS_MAX_ARGS), BOOST_ACCUMULATORS_ACCUMULATOR_SET_FUN_OP, _)
                ...
                struct apply : fusion::result_of::value_of<typename fusion::result_of::find_if<           \
                    accumulators_type, detail::matches_feature<Feature> >::type>{
                };
                ...
                void drop(){
                    ...
                    typedef typename feature_of<typename as_feature<Feature>::type>::type the_feature;
        ...
        typename mpl::apply<AccumulatorSet, Feature>::type& find_accumulator(                          \
                AccumulatorSet& acc BOOST_ACCUMULATORS_PROTO_DISABLE_IF_IS_CONST(AccumulatorSet)){
        ...
        // \overload
        template<typename Feature, typename AccumulatorSet>
        typename mpl::apply<AccumulatorSet, Feature>::type const& find_accumulator(AccumulatorSet const& acc){
        ...
        #define BOOST_ACCUMULATORS_EXTRACT_RESULT_FUN(z, n, _)
            template<typename Feature, typename AccumulatorSet BOOST_PP_ENUM_TRAILING_PARAMS_Z(z, n, typename A)>

            typename mpl::apply<AccumulatorSet, Feature>::type::result_type extract_result(             \
                AccumulatorSet const& acc BOOST_PP_ENUM_TRAILING_BINARY_PARAMS_Z(z, n, A, const &a)){

                return find_accumulator<Feature>(acc.)result(detail::accumulator_params()(
                            acc BOOST_PP_ENUM_TRAILING_PARAMS_Z(z, n, a)));
            }


        BOOST_PP_REPEAT(BOOST_PP_INC(BOOST_ACCUMULATORS_MAX_ARGS), BOOST_ACCUMULATORS_EXTRACT_RESULT_FUN, _)
