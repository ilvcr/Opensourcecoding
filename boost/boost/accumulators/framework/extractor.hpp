...
> Created Time: 2018年07月29日 星期日 21时13分34秒
> Description: 
...
namespace boost{
    namespace accumulators{
        namespace detail{
            ...
            struct argument_pack_result : accumulator_set_result<typename remove_reference<   \
                    typename parameter::binding<Args, tag::accumulator>::type>::type, Feature>{
            };

            
            template<typename A, typename Feature>
            struct extractor_result : mpl::eval_if<detail::is_accumulator_set<A>, accumulator_set_result<A, Feature>,   \
        ...
        struct extractor{
            ...
            struct result<this_type(A1)> : detail::extractor_result<A1, Feature>{
            };
            ...
            // INTERNAL ONLY
            #define BOOST_ACCUMULATORS_EXTRACTOR_FUN_OP(z, n, _)  template<BOOST_PP_ENUM_PARAMS_Z(z, n, typename A)>

            struct result<this_type(BOOST_PP_ENUM_PARAMS_Z(z, n, A))> : detail::extractor_result<A1, Feature>{
            };


            template<typename AccumulatorSet BOOST_PP_ENUM_TRAILING_PARAMS_Z(z, n, typename A)>

            typename detail::extractor_result<AccumulatorSet, Feature>::type
            operator ()(AccumulatorSet const &acc BOOST_PP_ENUM_TRAILING_BINARY_PARAMS_Z(z, n, A, const &a)) const{
            ...
            BOOST_PP_REPEAT_FROM_TO(2, BOOST_PP_INC(BOOST_ACCUMULATORS_MAX_ARGS), BOOST_ACCUMULATORS_EXTRACTOR_FUN_OP, _)
        ...
        #define BOOST_ACCUMULATORS_MAKE_FEATURE(Tag, Feature, ParamsSeq)
            Tag::Feature<BOOST_ACCUMULATORS_SEQ_REM(BOOST_PP_SEQ_TRANSFORM(BOOST_ACCUMULATORS_ARGS_OP, ~, ParamsSeq))>

        // INTERNAL ONLY
        #define BOOST_ACCUMULATORS_DEFINE_EXTRACTOR_FUN_IMPL(z, n, Tag, Feature, ParamsSeq)
        template<BOOST_ACCUMULATORS_SEQ_REM(BOOST_PP_SEQ_TRANSFORM(BOOST_ACCUMULATORS_PARAMS_OP, ~, ParamsSeq)), \
            typename Arg1 BOOST_PP_ENUM_TRAILING_PARAMS_Z(z, n, typename A)>

        typename boost::accumulators::detail::extractor_result<Arg1, 
            BOOST_ACCUMULATORS_MAKE_FEATURE(Tag, Feature, ParamsSeq)>::type
        ...
        Feature(Arg1 const &arg1 BOOST_PP_ENUM_TRAILING_BINARY_PARAMS_Z(z, n, A, const &a) ){
            ...
            return boost::accumulators::extractor<feature_type>()(arg1 BOOST_PP_ENUM_TRAILING_PARAMS_Z(z, n, a));
        }

        // INTERNAL ONLY
        #define BOOST_ACCUMULATORS_DEFINE_EXTRACTOR_FUN(z, n, _)
            BOOST_ACCUMULATORS_DEFINE_EXTRACTOR_FUN_IMPL(z, n, BOOST_PP_ARRAY_ELEM(0, _),  \
                BOOST_PP_ARRAY_ELEM(1, _), BOOST_PP_ARRAY_ELEM(2, _))
