/*************************************************************************
> File Name: functional_fwd.hpp
> Author: yoghourt->ilvcr 
> Mail: liyaoliu@foxmail.com  @@  ilvcr@outlook.com 
> Created Time: 2018年08月02日 星期四 20时33分30秒
> Description: 
...
namespace boost{
    namespace numeric{
        ...
        namespace functional{
            ...
            template<typename T>
            struct tag<T const> : tag<T>{
            };

            template<typename T>
            struct tag<volatile> : tag<T>{
            };

            template<typename T>
            struct tag<T const volatile> : tag<T>{
            };
...
#define BOOST_NUMERIC_FUNCTIONAL_DECLARE_BINARY_OP(Name)
        namespace functional{
            ...
            template<typename Left, typename Right, typename EnableIf = void>                       \
            struct Name ## _base;  
            template<typename Left, typename Right, typename LeftTag = typename tag<Left>::type, 
