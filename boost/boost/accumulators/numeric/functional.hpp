...
> Created Time: 2018年08月02日 星期四 20时55分08秒
> Description: 
...
namespace boost{
    namespace numeric{
        namespace functional{
            ...
            // INTERNAL ONLY
            template<typename AO, typename A1>
            struct are_integral : mpl::and_<is_integral<A0>, is_integral<A1> >{
            }
...
#define BOOST_NUMERIC_FUNCTIONAL_DEFINE_UNARY_OP(Name, Op)
        namespace functional{
            ...
            struct result_of_ ## Name{
                ...
                typeof typename nested::type type;
            };
            ...
            struct Name : Name ## _base<Arg, void>{
            };
        }

        namespace op{

            struct Name : boost::detail::function1<functional::Name<_, functional::tag<_> > >{
            };
        }
...
#define BOOST_NUMERIC_FUNCTIONAL_DEFINE_BINARY_OP(Name, Op, RetType) 
        namespace functional{
            ...
            struct Name : Name ## _base<Left, Right, void>{
            };
        }

        namespace op{

            struct Name : boost::detail::function2<functional::Name<_1, _2, 
                        functional::tag<_1>, functional::tag<_2> > >{                               \
            };
        }
...
#define BOOST_NUMERIC_FUNCTIONAL_DEDUCED(Left, Op, Right)
        BOOST_TYPEOF_NESTED_TYPEDEF_TPL(nested, boost::numeric::functional::detail::lvalue_of<Left>() Op   \
...
#undef BOOST_NUMERIC_FUNCTIONAL_DEFINE_BINARY_OP
        ...
        namespace functional{
            ...
            template<typename Left, typename Right, typename EnableIf>
            struct fdiv_base : functional::divides<Left, Right>{
            };
            ...
            struct promote : promote_base<To, From, void>{
            };

            template<typename Left, typename Right, typename LeftTag, typename RightTag>
            struct min_assign : min_assign_base<Left, Right, void>{
            };

            template<typename Left, typename Right, typename LeftTag, typename RightTag>
            struct max_assign : max_assign_base<Left, Right, void>{
            };


            template<typename Left, typename Right, typename LeftTag, typename RightTag>
            struct fdiv : fdiv_base<Left, Right, void>{
            };

            // INTERNAL ONLY 
            // For back-compat only. Use fdiv.
            template<typename Left, typename Right, typename LeftTag, typename RightTag>
            struct average : fdiv<Left, Right, LeftTag, RightTag>{
            };

            template<typename Arg, typename Tag>
            struct as_min : as_min_base<Arg, void>{
            };

            template<typename Arg, typename Tag>
            struct as_max : as_max_base<Arg, void>{
            };

            template<typename Arg, typename Tag>
            struct as_zero : as_zero_base<Arg, void>{
            };

            template<typename Arg, typename Tag>
            struct as_one : as_one_base<Arg, void>{
            };
        }
        ...
        template<typename T>
        struct one_or_default : mpl::if_<is_empty<T>, default_<T>, one<T> >::type{
        };

        template<typename T>
        struct zero_or_default : mpl::if_<is_empty<T>, default_<T>, one<T> >::type{
        };

        template<typename T>
        struct zero_or_default : mpl::if_<is_empty<T>, default_<T>, zero<T> >::type{
        };
