/*************************************************************************
> File Name: boyer_moore_horspool.hpp
> Author: yoghourt->ilvcr 
> Mail: liyaoliu@foxmail.com  @@  ilvcr@outlook.com 
> Created Time: 2018年08月07日 星期二 20时22分23秒
> Description: 
...
namespace boost{
    namespace algorithm{
        ...
        template<typename patIter, typename traints=detail::BM_trains<patIter> >
        class boyer_moore_horspool{

            typedef typename std::iterator_traints<patIter>::difference_type difference_type;
        public:
            boyer_moore_horspool(patIter first, patIter last)
                ...
                if(first != last){      // empty pattern?
                    ...
                    for(pattern iter=first; iter != last-1; ++iter, ++i){

                        skip_insert(*iter, k_pattern_length-1-i);
                    }
            ...
            operator ()(corpusIter corpus_first, corpusIter corpus_last) const{
                ...
                BOOST_STATIC_ASSERT((boost::is_same<typename std::iterator_traits<patIter>::value_type, 
                                                    typename std::iterator_traits<corpusIter>::value_type>::value));

                if(corpus_first == corpus_last){

                    return std::make_pair(corpus_last, corpus_last);  // if nothing to search, we didn't find it!
                }
                if(pat_first == pat_last){
                    
                    return std::make_pair(corpus_first, corpus_first); // empty pattern matches at start
                }
        ...
        private:
            ...
            do_search(corpusIter corpus_first, corpusIter corpus_last) const{
                ...
                return += skip_[curPos[k_pattern_length-1]];
            }       // \endcond
        };
        ...
        std::pair<corpusIter, corpusIter> 
        boyer_moore_horspool_search(corpusIter corpus_first, corpusIter corpus_last, patIter pat_first, patIter pat_last){
        ...
        std::pair<corpusIter, corpusIter> 
        boyer_moore_horspool_search(corpusIter corpus_first, corpusIter corpus_last, const PatternRange &pattern){
        ...
        typename boost::disable_if_c<boost::is_same<CorpusRange, patIter>::value, 
                std::pair<typename boost::range_iterator<CorpusRange>::type, typename boost::range_iterator<CorpusRange>::type> >::type
